    ...
    <div style={{ fontFamily: "var(--font-sans)", background: "#FDFAF5", color: "#1C1C1C" }}>
      ...
      <div style={{ background: "#F5F0E8", padding: "48px 32px 40px", borderBottom: "0.5px solid #D5C9B0" }}>
        <div style={{ fontSize: "11px", color: "#888780", marginBottom: "14px" }}>
          <Link href="/platforms" style={{ color: "#888780", textDecoration: "none" }}>Platforms</Link>
          <span style={{ color: "#2D6A4F" }}> / {p.name}</span>
        </div>

        {/* Platform name badge */}
        <div style={{ display: "inline-block", background: "#FFFFFF", border: `1px solid ${p.color}`, borderRadius: "4px", padding: "4px 12px", fontSize: "12px", fontWeight: 600, color: p.color, marginBottom: "16px", letterSpacing: "0.3px" }}>
          {p.name}
        </div>
      ...
      <section style={{ padding: "56px 32px", borderBottom: "0.5px solid #D5C9B0" }}>
        ...
        <p style={{ fontSize: "13px", color: "#555550", lineHeight: 1.65, maxWidth: "520px", marginBottom: "36px" }}>
          Every service below is available standalone or as part of a monthly managed retainer.
        </p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: "16px" }}>
          {p.services.map((s, i) => (
            <div key={s.title} style={{ background: i === 0 ? "#E8F5EE" : "#FFFFFF", border: `0.5px solid ${i === 0 ? "#9FE1CB" : "#D5C9B0"}`, borderRadius: "8px", padding: "22px" }}>
      ...
      <section style={{ padding: "56px 32px", background: "#F5F0E8", borderBottom: "0.5px solid #D5C9B0" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "48px", alignItems: "start" }}>
          ...
          {/* CTA card */}
          <div>
            <div style={{ background: "#FFFFFF", border: "0.5px solid #D5C9B0", borderRadius: "8px", padding: "28px", marginBottom: "16px" }}>
              <div style={{ fontSize: "13px", fontWeight: 500, color: "#1C1C1C", marginBottom: "6px" }}>
                Start with a free catalog audit
              </div>
              <div style={{ fontSize: "12px", color: "#888780", lineHeight: 1.6, marginBottom: "20px" }}>
                We review 50 of your {p.name} SKUs — data completeness, listing quality, and attribute accuracy — and deliver a written findings report within 48 hours.
              </div>
      ...
      <section style={{ padding: "40px 32px", borderBottom: "0.5px solid #D5C9B0" }}>
        <div style={{ fontSize: "10px", letterSpacing: "1.5px", color: "#2D6A4F", fontWeight: 500, marginBottom: "16px" }}>WE ALSO SUPPORT</div>
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
          {p.also.map((name) => {
            const otherSlug = Object.values(platforms).find((pl) => pl.name === name || pl.name.startsWith(name))?.slug;
            return otherSlug ? (
              <Link key={name} href={`/platforms/${otherSlug}`} style={{ border: "0.5px solid #D5C9B0", borderRadius: "3px", padding: "7px 16px", fontSize: "13px", color: "#555550", background: "#FDFAF5", textDecoration: "none" }}>
                {name}
              </Link>
            ) : (
              <div key={name} style={{ border: "0.5px solid #D5C9B0", borderRadius: "3px", padding: "7px 16px", fontSize: "13px", color: "#555550", background: "#FDFAF5" }}>
                {name}
              </div>
            );
          })}
