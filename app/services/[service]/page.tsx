    ...
    <div style={{ fontFamily: "var(--font-sans)", background: "#FDFAF5", color: "#1C1C1C" }}>
      ...
      <div style={{ background: "#F5F0E8", padding: "48px 32px 40px", borderBottom: "0.5px solid #D5C9B0" }}>
        <div style={{ fontSize: "11px", color: "#888780", marginBottom: "14px" }}>
          <Link href="/services" style={{ color: "#888780", textDecoration: "none" }}>Services</Link>
          <span style={{ color: "#2D6A4F" }}> / {s.name}</span>
        </div>
        <div style={{ fontSize: "10px", letterSpacing: "1.5px", color: "#2D6A4F", fontWeight: 500, marginBottom: "14px" }}>
          EXECUTION SERVICE
        </div>
      ...
      <div style={{ background: "#2D6A4F", padding: "20px 32px", display: "grid", gridTemplateColumns: "repeat(4,1fr)" }}>
        {s.stats.map((stat) => (
          <div key={stat.l} style={{ textAlign: "center" }}>
            <div style={{ fontFamily: "var(--font-serif)", fontSize: "22px", color: "#fff", fontWeight: 500 }}>{stat.n}</div>
            <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.7)", marginTop: "2px" }}>{stat.l}</div>
          </div>
        ))}
      </div>
      ...
      <section style={{ padding: "56px 32px", borderBottom: "0.5px solid #D5C9B0" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "48px", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: "10px", letterSpacing: "1.5px", color: "#2D6A4F", fontWeight: 500, marginBottom: "10px" }}>WHY IT MATTERS</div>
            <h2 style={{ fontFamily: "var(--font-serif)", fontSize: "26px", fontWeight: 500, color: "#1C1C1C", marginBottom: "16px" }}>
              The operational cost of getting this wrong.
            </h2>
            <p style={{ fontSize: "14px", color: "#555550", lineHeight: 1.75 }}>{s.whyItMatters}</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            {s.stats.map((stat) => (
              <div key={stat.l} style={{ background: "#F5F0E8", border: "0.5px solid #D5C9B0", borderRadius: "8px", padding: "20px", textAlign: "center" }}>
                <div style={{ fontFamily: "var(--font-serif)", fontSize: "28px", fontWeight: 500, color: "#2D6A4F" }}>{stat.n}</div>
      ...
      <section style={{ padding: "56px 32px", background: "#F5F0E8", borderBottom: "0.5px solid #D5C9B0" }}>
        <div style={{ fontSize: "10px", letterSpacing: "1.5px", color: "#2D6A4F", fontWeight: 500, marginBottom: "10px" }}>WHAT WE DO</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: "16px" }}>
          {s.items.map((item, i) => (
            <div key={item.title} style={{ background: i === 0 ? "#E8F5EE" : "#FFFFFF", border: `0.5px solid ${i === 0 ? "#9FE1CB" : "#D5C9B0"}`, borderRadius: "8px", padding: "22px" }}>
              <div style={{ fontSize: "14px", fontWeight: 500, color: "#1C1C1C", marginBottom: "8px" }}>{item.title}</div>
              <div style={{ fontSize: "12px", color: "#555550", lineHeight: 1.65 }}>{item.body}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: "56px 32px", borderBottom: "0.5px solid #D5C9B0" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: "48px", alignItems: "start" }}>
          <div>
            ...
            <h2 style={{ fontFamily: "var(--font-serif)", fontSize: "26px", fontWeight: 500, color: "#1C1C1C", marginBottom: "24px" }}>
            ...
            {/* Platforms */}
            <div style={{ fontSize: "10px", letterSpacing: "1.5px", color: "#2D6A4F", fontWeight: 500, marginBottom: "12px" }}>PLATFORMS</div>
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
              {s.platforms.map((p) => (
                <div key={p} style={{ border: "0.5px solid #D5C9B0", borderRadius: "3px", padding: "5px 12px", fontSize: "12px", color: "#555550", background: "#FDFAF5" }}>{p}</div>
              ))}
            </div>
          </div>
          ...
          {/* CTA sidebar */}
          <div>
            <div style={{ background: "#FFFFFF", border: "0.5px solid #D5C9B0", borderRadius: "8px", padding: "24px", marginBottom: "14px" }}>
              ...
              <Link href="/audit" style={{ display: "block", background: "#2D6A4F", color: "#fff", padding: "12px 20px", borderRadius: "4px", fontSize: "13px", fontWeight: 500, textAlign: "center", textDecoration: "none", marginBottom: "10px" }}>
                Get free catalog audit
              </Link>
              <Link href="/pricing" style={{ display: "block", border: "0.5px solid #D5C9B0", color: "#555550", padding: "10px 20px", borderRadius: "4px", fontSize: "12px", textAlign: "center", textDecoration: "none" }}>
                View pricing
              </Link>
            </div>
            <div style={{ background: "#E8F5EE", border: "0.5px solid #9FE1CB", borderRadius: "6px", padding: "14px" }}>
              <div style={{ fontSize: "11px", color: "#085041", lineHeight: 1.6 }}>
                <span style={{ fontWeight: 500 }}>No lock-in.</span> All services available standalone or as part of a monthly retainer. Scale up or down as needed.
              </div>
            </div>
          </div>
        </div>
      ...
      {s.relatedServices.length > 0 && (
        <section style={{ padding: "40px 32px", background: "#F5F0E8", borderBottom: "0.5px solid #D5C9B0" }}>
          <div style={{ fontSize: "10px", letterSpacing: "1.5px", color: "#2D6A4F", fontWeight: 500, marginBottom: "16px" }}>RELATED SERVICES</div>
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
            {s.relatedServices.map((relSlug) => {
              const rel = services[relSlug];
              if (!rel) return null;
              return (
                <Link key={relSlug} href={`/services/${relSlug}`} style={{ background: "#FFFFFF", border: "0.5px solid #D5C9B0", borderRadius: "6px", padding: "12px 18px", textDecoration: "none", display: "block" }}>
                  <div style={{ fontSize: "13px", fontWeight: 500, color: "#1C1C1C", marginBottom: "2px" }}>{rel.name}</div>
                  <div style={{ fontSize: "11px", color: "#888780" }}>{rel.tagline}</div>
                </Link>
              );
            })}
          </div>
      ...
      <div style={{ background: "#1C1C1C", padding: "48px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "24px" }}>
        <div>
          ...
          <div style={{ fontSize: "13px", color: "rgba(255,255,255,0.6)", maxWidth: "440px", lineHeight: 1.7 }}>
            Start with a free catalog audit. We&apos;ll review your store and show you exactly what we&apos;d fix first.
          </div>
        </div>
