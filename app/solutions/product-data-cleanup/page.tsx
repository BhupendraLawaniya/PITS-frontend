...
export const metadata: Metadata = {
  ...
  keywords: ["product data cleanup", "catalog deduplication", "ecommerce data audit", "product data enrichment", "SKU cleanup"],
  openGraph: {
    title: "Product Data Cleanup & Optimisation – Professional ITS",
    description: "We audit, deduplicate, enrich, and restructure your product data at any scale.",
...
const problems = [
  { title: "Duplicate SKUs", body: "Same product listed multiple times with different IDs, titles, or attributes — confusing buyers and splitting your sales." },
  { title: "Missing attributes", body: "Blank fields, missing specs, empty filter values — products that can't be found in search." },
  { title: "Wrong categories", body: "Products in the wrong Browse Node or category tree — invisible to buyers browsing by category." },
  { title: "Inconsistent formatting", body: "Mixed units, inconsistent capitalisation, non-standard values — breaking filters and bulk exports." },
];

const process = [
  { n: "01", title: "Full audit", body: "We score your catalog across completeness, accuracy, and formatting — every SKU gets a pass/fail on each dimension." },
  { n: "02", title: "Deduplication", body: "Duplicate detection using title, ASIN, EAN, and attribute matching. Merge or remove — your choice." },
  { n: "03", title: "Enrichment", body: "Missing attributes filled using AI-assisted extraction from datasheets, PDFs, and manufacturer pages." },
  { n: "04", title: "Restructure", body: "Categories corrected, attributes standardised, formatting normalised — platform-ready output." },
  { n: "05", title: "QA & report", body: "Post-cleanup accuracy check. Written findings report delivered with before/after scoring." },
];
    ...
    <div style={{ fontFamily: "var(--font-sans)", background: "#FDFAF5", color: "#1C1C1C" }}>
      ...
      <div style={{ background: "#F5F0E8", padding: "48px 32px 40px", borderBottom: "0.5px solid #D5C9B0" }}>
        ...
        <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "36px", fontWeight: 500, lineHeight: 1.15, color: "#1C1C1C", marginBottom: "14px" }}>
          Messy data is costing you <em style={{ color: "#2D6A4F", fontStyle: "italic" }}>sales.</em>
        </h1>
        <p style={{ fontSize: "14px", color: "#555550", lineHeight: 1.75, maxWidth: "580px", marginBottom: "28px" }}>
          We audit, deduplicate, enrich, and restructure your product data — whether it&apos;s 200 SKUs or 200,000. We work with feeds, flat files, and direct platform uploads.
        </p>
      ...
      <section style={{ padding: "56px 32px", borderBottom: "0.5px solid #D5C9B0" }}>
        <div style={{ fontSize: "10px", letterSpacing: "1.5px", color: "#2D6A4F", fontWeight: 500, marginBottom: "10px" }}>THE PROBLEM</div>
        ...
        <p style={{ fontSize: "13px", color: "#555550", lineHeight: 1.65, maxWidth: "520px", marginBottom: "36px" }}>
          Most sellers don&apos;t realise how much bad data is hurting their visibility and conversion. These are the four most common issues we find.
        </p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: "16px" }}>
          {problems.map((p) => (
            <div key={p.title} style={{ background: "#FDF8F0", border: "0.5px solid #EDE5D5", borderRadius: "8px", padding: "20px", borderLeft: "3px solid #D5C9B0" }}>
              <div style={{ fontSize: "14px", fontWeight: 500, color: "#1C1C1C", marginBottom: "8px" }}>{p.title}</div>
              <div style={{ fontSize: "13px", color: "#555550", lineHeight: 1.6 }}>{p.body}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: "56px 32px", background: "#F5F0E8", borderBottom: "0.5px solid #D5C9B0" }}>
        <div style={{ fontSize: "10px", letterSpacing: "1.5px", color: "#2D6A4F", fontWeight: 500, marginBottom: "10px" }}>OUR PROCESS</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: "12px" }}>
          {process.map((s, i) => (
            <div key={s.n} style={{ position: "relative" }}>
              <div style={{ fontFamily: "var(--font-serif)", fontSize: "32px", color: "#D5C9B0", fontWeight: 500, marginBottom: "10px" }}>{s.n}</div>
              <div style={{ fontSize: "13px", fontWeight: 500, color: "#1C1C1C", marginBottom: "6px" }}>{s.title}</div>
              <div style={{ fontSize: "12px", color: "#555550", lineHeight: 1.6 }}>{s.body}</div>
              {i < process.length - 1 && (
                <div style={{ position: "absolute", top: "16px", right: "-6px", fontSize: "16px", color: "#D5C9B0" }}>→</div>
              )}
            </div>
          ))}
        </div>
      ...
      <section style={{ padding: "40px 32px", borderBottom: "0.5px solid #D5C9B0" }}>
        <div style={{ fontSize: "10px", letterSpacing: "1.5px", color: "#2D6A4F", fontWeight: 500, marginBottom: "16px" }}>WHAT&apos;S INCLUDED</div>
      ...
      <div style={{ background: "#1C1C1C", padding: "48px 32px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <div style={{ fontSize: "13px", color: "rgba(255,255,255,0.6)", maxWidth: "440px", lineHeight: 1.7 }}>
            Free 50-SKU audit. Written findings report in 48 hours. We&apos;ll show you exactly what we&apos;d fix first.
          </div>
        </div>
